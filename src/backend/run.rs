...
// winkel --backend: one json line per request on stdin, one json line per event
...
// One Winkel at a time: two metronomes playing the same bar a beat-length
...
pub fn lock_path() -> std::path::PathBuf {
    ...
    let name = match uid {
        Some(id) => format!("winkel-{}.lock", id),
        None => "winkel.lock".to_string(),
    };
...
// Try to become the one Winkel. Ok(file) holds the lock for the process's
...
pub fn acquire_instance_lock() -> Result<std::fs::File, (&'static str, String)> {
    ...
    file.try_lock().map(|()| file).map_err(|err| match err {
        std::fs::TryLockError::WouldBlock => (
            "already_running",
            format!("Winkel is already running ({})", path.display()),
        ),
...
pub fn run() -> i32 {
    ...
    // WINKEL_SILENT forces the silent clock, so a headless box or a test can
    // exercise the whole protocol without an audio device in the room.
    let silent_forced = std::env::var_os("WINKEL_SILENT").is_some_and(|v| !v.is_empty());
